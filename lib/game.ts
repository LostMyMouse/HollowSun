...
export function bonus(g:Game,skill:string){const main=g.role==='Gunslinger'?'Combat':g.role==='Diplomat'?'Presence':'Tech';const attribute:Record<string,Ability>={Combat:'Dexterity',Presence:'Charisma',Tech:'Intelligence',Agility:'Dexterity',Insight:'Wisdom'};return modifier(score(g,attribute[skill]??'Wisdom'))+(skill===main?3:1)+(g.xp>=60?1:0);}
...
export function infer(text:string,g:Game):Plan{
 ...
 if(/\b(shoot|attack|strike|punch|stab|melee|blast|fire at|kill)\b/.test(t)){intent='attack';skill='Combat';difficulty=12;}
...
export function resolve(original:Game,plan:Plan,input:string,rollDie=dice):{game:Game;facts:string;roll?:Roll}{
 ...
 if(intent==='hack'){dc=12;skill='Tech';}
 if(intent==='sneak'){dc=12;skill='Agility';}
 if(intent==='recruit'){dc=g.flags.clue?8:11;skill='Presence';}
 if(intent==='persuade'){dc=target==='veyra'?17:12;skill='Presence';}
 if(intent==='flee'){dc=10;skill='Agility';}
 ...
 else if(target==='brakk'){speaker='Brakk';award(g,'weakness',10);text='“Veyra’s command rig borrows power from the artifact. Cut the security terminal’s drone uplink before you go up. Or disconnect the relay in her chamber.” He lowers his voice. “Bring the courier out alive.”';}
 ...
 else if(target==='veyra'){speaker='Director Veyra';text=g.flags.bossDefeated?'Her command rig lies broken. The artifact waits for your decision: destroy it, claim it, or spare Veyra and give the gates to the station.':'“You think a station survives on freedom? It survives on someone willing to collect.” Her rig turns toward you. Fight her, or negotiate from a position of strength.';}
 else if(target==='relay')text=g.flags.sabotage?'The relay is disabled. Veyra’s rig is exposed.':'Alien energy courses through the relay. Hacking it could strip away Veyra’s support.';
 ...
 else text='The lift leads to Veyra’s reactor chamber. Bring the artifact if you want leverage, or enter now and face her directly.';
 ...
 else if(target==='veyra'){if(g.flags.artifact&&(g.flags.ally||g.flags.sabotage)&&success){award(g,'bossDefeated',60);g.combat=false;g.enemies=[];text='With the artifact in your hands and her support compromised, Veyra finally disconnects her rig. “Name your terms.” Her power is broken. Decide the artifact’s fate.';}else{text='Veyra refuses. “Words without leverage.” Bring the artifact and either turn Korr or sabotage her network to force a negotiated surrender.';}}
 ...
 consumed=true;if(success){const damage=rollDie(8)+4+(roll?.die===20?6:0)+(g.role==='Gunslinger'?2:0);victim.hp=Math.max(0,victim.hp-damage);text=`Your shot hits ${victim.name} for ${damage} damage.`;}else text=`Your shot misses ${victim.name} and scorches the decking.`;
...
export function suggestions(g:Game){if(g.flags.bossDefeated&&!g.ending)return ['Destroy the artifact','Claim the syndicate','Spare Veyra; give the gates to the station'];if(g.combat)return ['Attack','Take cover','Use a medkit','Retreat'];const m:Record<string,string[]>={nyx:['Ask about Sol Renn','Ask Nyx to join my crew'],brakk:['Ask about Veyra’s weakness'],pip:['Ask PIP about the courier'],guard:['Persuade Korr to stand down','Attack Korr'],cargo:['Investigate the cargo vault','Sneak into the vault'],terminal:['Hack the security terminal'],ship:['Rest aboard my ship'],lift:['Enter the reactor lift'],veyra:['Talk to Veyra','Attack Veyra','Convince Veyra to surrender'],relay:['Disconnect the relay'],exit:['Return to the station']};return m[g.target]??['Look around'];}
